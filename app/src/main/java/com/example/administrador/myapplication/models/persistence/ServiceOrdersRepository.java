...
public final class ServiceOrdersRepository {
    ...
    public void delete(ServiceOrder serviceOrder) {
        ...
        content.put(ServiceOrderContract.ACTIVE, 0);
        db.update(ServiceOrderContract.TABLE, content, where, args);
        db.close();
        helper.close();
    }
    ...
    public List<ServiceOrder> getList(boolean isActive) {
        DatabaseHelper helper = new DatabaseHelper(AppUtil.CONTEXT);
        SQLiteDatabase db = helper.getReadableDatabase();
        String where = null;
        if (isActive) {
            where = ServiceOrderContract.ACTIVE + "=1";
        }
